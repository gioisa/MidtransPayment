...
namespace Midtrans.Payment.Core.Guest.Query
{
    public class GetGuestListRequest : ListRequest,IListRequest<GetGuestListRequest>,IRequest<ListResponse<GuestResponse>>
    {
    }
    internal class GetGuestListHandler : IRequestHandler<GetGuestListRequest, ListResponse<GuestResponse>>
    {
        private readonly ILogger _logger;
        private readonly IMapper _mapper;
        private readonly IUnitOfWork<ApplicationDBContext> _context;
        public GetGuestListHandler(
            ILogger<GetGuestListHandler> logger,
            IMapper mapper,
        ...
        public async Task<ListResponse<GuestResponse>> Handle(GetGuestListRequest request, CancellationToken cancellationToken)
        {
            ListResponse<GuestResponse> result = new ListResponse<GuestResponse>();
            try
            {
				var query = _context.Entity<Midtrans.Payment.Data.Model.MstGuest>().AsQueryable();

				#region Filter
				Expression<Func<Midtrans.Payment.Data.Model.MstGuest, object>> column_sort = null;
				List<Expression<Func<Midtrans.Payment.Data.Model.MstGuest, bool>>> where = new List<Expression<Func<Midtrans.Payment.Data.Model.MstGuest, bool>>>();
				...
				result.List = _mapper.Map<List<GuestResponse>>(data_list);
            ...
            {
                _logger.LogError(ex, "Failed Get List Guest", request);
                result.Error("Failed Get List Guest", ex.Message);
            }
            return result;
        }

        #region List Utility
		private (Expression<Func<Midtrans.Payment.Data.Model.MstGuest, bool>> where, Expression<Func<Midtrans.Payment.Data.Model.MstGuest, object>> order) ListExpression(string search, string field, bool is_where)
		{
			Expression<Func<Midtrans.Payment.Data.Model.MstGuest, object>> result_order = null;
			Expression<Func<Midtrans.Payment.Data.Model.MstGuest, bool>> result_where = null;
